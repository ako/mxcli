...
| Disabled activity | `@disabled` before an activity | Studio Pro's right-click **Disable**: the step stays in the flow, drawn greyed out, and is skipped at runtime. Mendix stores the flag on `Microflows$ActionActivity` and on **no other** microflow object, so it is refused (**MDL087**) on an `if`, `case`, `split type`, `loop`, `while`, `merge`, `join`, `return`, `raise error`, `break` or `continue` rather than dropped. `@excluded` before a *statement* is the older spelling and still sets it; before a `create microflow` the same word means "Exclude from project", a different setting (#1139) |
| Disable activities in place | `alter microflow\|nanoflow\|rule Mod.Name disable\|enable activities where <filter>` / `alter microflows\|nanoflows\|rules [in Mod] …` | Studio Pro's Disable on flows that **already exist** — `@disabled` states the flag while *authoring* one. Sets one boolean on the **stored** document rather than rewriting it, so a flow holding constructs MDL cannot spell survives untouched. Omitting `IN` is the whole project, as with `ALTER PAGES … SET LAYOUT`. Filter columns: `action` (MDL keyword, quoted when it has a space, or the storage name `CATALOG.ACTIVITIES.ActionType` prints), `level` (log only), `caption` (also `LIKE`), `disabled`; joined by `AND`, with `=`, `!=`, `IN (…)`, `NOT IN (…)`. A filter that cannot select anything is **MDL088** rather than a run reporting zero — see `mxcli syntax microflow.disable` (#1139) |
