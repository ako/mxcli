...
| Region | `region top \| right \| bottom \| left \| center [( size: N, sizemode: 'Fixed'\|'Pixels'\|'Auto', togglemode: '…', class: '…' )] { <widgets> }` | Five named slots, not a list. One region per slot |
| Region toggle | `togglemode: 'None' \| 'PushContentAside' \| 'SlideOverContent' \| 'ShrinkContentInitiallyOpen' \| 'ShrinkContentInitiallyClosed'` | Whether the region collapses — the difference between a sidebar and a fixed strip of chrome, and invisible on a desktop: without one a 232px sidebar keeps its width on a 414px phone. Atlas sets a different value on each of its sidebar layouts. Stored members, **not** Studio Pro's captions; a caption is refused (MDL-WIDGET29), since Mendix drops an unknown member on load and the layout still builds at 0 errors |
