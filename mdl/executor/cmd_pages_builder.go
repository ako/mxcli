...
func (pb *pageBuilder) initPluggableEngine() {
	...
	if pb.backend != nil {
		// Auto-refresh project-local defs whose generatorVersion is behind the
		// current build — otherwise an mxcli upgrade silently emits stale BSON
		// from defs generated by an older version (the .mpk hasn't changed).
		if refreshed, refreshErr := RefreshStaleWidgetDefinitions(pb.backend.Path()); refreshErr != nil {
			log.Printf("warning: refreshing stale widget definitions: %v", refreshErr)
		} else if refreshed {
			log.Printf("info: refreshed stale widget definitions for %s", pb.backend.Path())
		}
