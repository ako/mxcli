...
type EntityMember struct {
	...
	Ref       string
	Inherited bool
	// IsCalculated and IsAutoNumber are the two shapes Mendix refuses write
	// access on (CE6592). They are separate flags rather than one because they
	// are unrelated in the model — an autonumber carries no CalculatedValue —
	// and asking whether write is allowed is types.WriteRightsForbidden's job,
	// not a caller's.
	IsCalculated bool
	IsAutoNumber bool
}
...
// isAutoNumberAttr reports whether an attribute's value is generated by the
// database on insert. Mendix refuses write rights on one (CE6592), the same as
// for a calculated attribute and for the same reason, but the two look nothing
// alike in the model: this is a property of the attribute's TYPE, where
// calculated is a property of its VALUE.
func isAutoNumberAttr(attr *domainmodel.Attribute) bool {
	if attr == nil {
		return false
	}
	_, ok := attr.Type.(*domainmodel.AutoNumberAttributeType)
	return ok
}
