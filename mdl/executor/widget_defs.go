...
// RefreshStaleWidgetDefinitions cheaply checks whether any project-local
// `.mxcli/widgets/*.def.json` was generated by an older mxcli build (its
// `generatorVersion` stamp is behind WidgetDefGeneratorVersion) and, if so,
// triggers a full RefreshWidgetDefinitions. Returns true if a refresh ran.
//
// This is the `exec`-path guard: when mxcli's def-generation logic changes
// but the project's `.mpk` files don't, the existing defs would otherwise
// silently produce stale BSON (the case that surfaced as spurious CE0463
// after the v0.12.0 widget work). The check reads only the stamp from each
// def.json — no `.mpk` parsing — so the common up-to-date case is cheap;
// the expensive regenerate-and-compare only runs when something is behind.
func RefreshStaleWidgetDefinitions(projectPath string) (bool, error) {
	...
	if err != nil {
		// No project-local defs (fresh project / nothing extracted yet) — nothing to refresh.
		return false, nil
	}
