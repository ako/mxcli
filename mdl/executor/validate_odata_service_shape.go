...
//
// The two modes are historical: OData v3 had no link support, so a foreign key
// had to be an exposed object id; v4 added links largely so internal ids no
// longer had to leave the app. Which is why CE7375's own advice — publish the
// ID and make it the key — is the wrong direction for a shared contract: object
// ids are autogenerated and differ per environment, so one baked into an
// external contract breaks the moment a consumer moves between them.
//
...
func associationModeViolation(where string, publishesEntities bool) linter.Violation {
	v := linter.Violation{
		...
		Suggestion: "That representation requires the system ID attribute published as the key, " +
			...
			"default when the property is omitted. Do not follow CE7375 literally and publish the " +
			"ID instead: object ids are autogenerated and differ between test, acceptance and " +
			"production, so an id in an external contract breaks when a consumer changes " +
			"environment. Key on a business value.",
	}
