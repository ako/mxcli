...
- **`mxcli check` no longer recommends the statement that silently discards your `ALTER PAGE` edits** (ako/mxcli-maintenance §4) — a page column added with `ALTER PAGE` in one script vanished when another script's `create or replace page` was re-run, with nothing reported. The dropping is what CREATE OR REPLACE means and is not the defect; the advice was. For every document type except entity, check answered "page already exists in project: X — **use CREATE OR MODIFY to update it**", and for a page CREATE OR MODIFY is not a modify at all: the executor treats it identically to CREATE OR REPLACE and rebuilds the document from the statement.

  Page, snippet, layout and workflow now get the steer entities have had since an earlier finding of the same shape — the hint names the incremental `ALTER` for that type, says it leaves the rest intact, and says that CREATE OR MODIFY rebuilds the whole document and drops anything it does not mention, including edits made by ALTER. A type with no incremental ALTER (a microflow, say) keeps the generic hint rather than being invented one.
