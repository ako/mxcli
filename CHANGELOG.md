...
- **`CREATE OR REPLACE NAVIGATION` can now create a profile, not only replace one** (ako/mxcli-maintenance §7) — `create or replace navigation Phone` failed with `navigation profile not found: Phone (available: Responsive)`, and nothing else in mxcli added one. Mendix routes a phone to a Phone **profile** on the User-Agent rather than on viewport width, so this put a whole class of app out of reach: phone-specific pages existed but no phone browser could land on them, and a layout naming the missing profile failed CE1613. A profile is now created when the name is one of Mendix's fixed web kinds — Responsive, Phone, Tablet — and re-running the statement updates it as before.

  The shape is **pinned against a Studio Pro document carrying all three profiles**, read over PED, rather than derived from the Responsive one already in the project. That distinction is the whole of it: all three store the same fourteen keys, but only **Phone** carries `ProgressiveWebAppSettings`, so the obvious implementation — clone the sibling, change the name — produces a Phone profile with no PWA settings that `mx check` reports as 0 errors. An invented name such as `Mobile` is still an error, because a profile Mendix does not define can never route; Offline and native kinds are excluded for want of a reference document, and the legacy engine refuses rather than approximating.
