...
| Studio Pro shows a dropdown / property as its default value even though MDL set it explicitly (e.g. CREATE ODATA CLIENT with `ConfigurationMicroflow:` set, but the "Configuration source" dropdown reads "Constants only") | Mendix renamed the BSON storage key between versions; mxcli still writes the old name and Studio Pro silently ignores unknown keys | The `serializeXxx` function in `sdk/mpr/writer_*.go` for the affected document type | Dump the BSON of a Studio Pro-saved example of each dropdown state (`mpr.Open` + `ListRawUnitsByType` + `GetRawUnit` then JSON-marshal); diff against mxcli's output to find the renamed key. Update writer + parser to use the current key. The Go struct field can keep its old name as a documentation anchor with a comment pointing at the BSON name (e.g. `ConfigurationMicroflow string // BSON: ConfigurationEntityMicroflow`). If the dropdown has multiple mutually-exclusive states, each state usually corresponds to a different BSON key being non-empty (no separate discriminator field). Issue #573 follow-up |
