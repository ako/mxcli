...
- Warm local dev loop (`mxcli run --local [--watch]`): Docker-free `mxbuild --serve` + standalone runtime, hot `reload_model` for behavioural changes and restart+DDL for structural ones (chosen from the serve build's `restartRequired`). Bundles the browser client (`web/dist/` via mxbuild's rollup runner, which the serve Deploy target skips) so Mendix 11.x apps render in a browser (verified with Playwright). `--watch` re-bundles the client each change (~6-7s full bundle; incremental is a follow-up) — see `docs/11-proposals/PROPOSAL_mxcli_dev_warm_loop.md`
