...
| **Project brain** | `mxcli brain init\|capture\|staged\|promote\|drop\|check\|show` | Opt-in store in `docs/brain/` for what mxcli **cannot** compute (why a pattern was chosen here, which marketplace version broke what). Sharded by module — an entry's first anchor names its file — so a session loads `project.md` plus the modules it is touching, not the whole store. An agent captures to a git-ignored queue; a person promotes |
