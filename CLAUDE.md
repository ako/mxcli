...
- Warm local dev loop (`mxcli run --local [--watch]`): Docker-free `mxbuild --serve` + standalone runtime, hot `reload_model` for behavioural changes and restart+DDL for structural ones (chosen from the serve build's `restartRequired`). Boots Mendix 11.x apps to HTTP 200; browser client bundle (`web/dist/`) not yet produced by the serve path — see `docs/11-proposals/PROPOSAL_mxcli_dev_warm_loop.md`
